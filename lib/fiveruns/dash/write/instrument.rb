module Fiveruns::Dash::Write::Instrument
  ...
  def self.add(*raw_targets, &handler)
    ...
    raw_targets.each do |raw_target|
      ...
      rescue Fiveruns::Dash::Write::Instrument::Error => em
        raise em
      rescue => e
    ...
    begin
      ...
      if Thread.current[token] == 0
        if !limit_to_within || (Thread.current[:dash_markers] || []).include?(limit_to_within)
          Fiveruns::Dash::Write::Instrument.handlers[offset].call(this, time, *args)
        end
      end
    end
    result
  ...
  def self.instrument(obj, meth, options = {}, &handler)
    ...
    code = wrapping meth, identifier do |without|
      if options[:exceptions]
        ...
        <<-REENTRANT
          Fiveruns::Dash::Write::Instrument.reentrant_timing(:id#{options[:reentrant_token]}, #{offset}, self, #{options[:only_within] ? ":#{options[:only_within]}" : 'nil'}, args) do
            #{without}(*args, &block)
          end
        REENTRANT
      else
        <<-PERFORMANCE
          Fiveruns::Dash::Write::Instrument.timing(#{offset}, self, args, #{options[:mark_as] ? ":#{options[:mark_as]}" : 'nil'}, #{options[:only_within] ? ":#{options[:only_within]}" : 'nil'}) do
            #{without}(*args, &block)
          end
        PERFORMANCE
      end
