...
if RUBY_PLATFORM[/java/]
  ...
  Fiveruns::Dash.recipe :jruby, :url => 'http://dash.fiveruns.com' do |metrics|
    ...
    metrics.absolute :live_class_count, "Live Class Count", "Number of active classes" do 
      Integer(Fiveruns::Dash::Write::JRuby.mbean("ClassCache", ClassCacheMBean::java_class).get_live_class_count)
    end
    metrics.absolute :class_load_count, "Class Load Count", "Number of loaded classes" do 
      Integer(Fiveruns::Dash::Write::JRuby.mbean("ClassCache", ClassCacheMBean::java_class).get_class_load_count)
    end
    metrics.absolute :class_reuse_count, "Class Reuse Count", "Number of reused classes" do 
      Integer(Fiveruns::Dash::Write::JRuby.mbean("ClassCache", ClassCacheMBean::java_class).get_class_reuse_count)
    end
    ...
    metrics.absolute :average_code_size, "Average Code Size", "Average Code Size" do 
      Integer(Fiveruns::Dash::Write::JRuby.mbean("JITCompiler", JITCompilerMBean::java_class).average_code_size)
    end
    metrics.absolute :compile_count, "Compile Count", "Compile Count" do 
      Integer(Fiveruns::Dash::Write::JRuby.mbean("JITCompiler", JITCompilerMBean::java_class).compile_count)
    end
    metrics.absolute :compile_success_count, "Compile Success Count", "Compile Success Count" do 
      Integer(Fiveruns::Dash::Write::JRuby.mbean("JITCompiler", JITCompilerMBean::java_class).success_count)
    end
    metrics.absolute :compile_fail_count, "Compile Fail Count", "Compile Fail Count" do 
      Integer(Fiveruns::Dash::Write::JRuby.mbean("JITCompiler", JITCompilerMBean::java_class).fail_count)
    end
    metrics.absolute :average_compile_time, "Average Compile Time", "Average Compile Time" do 
      Integer(Fiveruns::Dash::Write::JRuby.mbean("JITCompiler", JITCompilerMBean::java_class).average_compile_time)
    end
